...
import { Clock, Trophy, Palette, Dice5, CheckCircle, Loader2 } from 'lucide-react'
    ...
    <section id="events" className="py-24 md:py-32 px-6 bg-forge-900/20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {events.map((evt, i) => (
            <ScrollReveal key={evt.day} delay={i * 0.05}>
              <div className="group bg-forge-900/40 border border-forge-800/50 hover:border-ember-500/25 rounded-xl overflow-hidden transition-all duration-300 card-hover flex flex-col h-full">
                <div className="relative h-36 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="text-ember-400 text-[10px] font-mono tracking-[0.15em] uppercase bg-forge-950/80 backdrop-blur-sm px-2.5 py-1 rounded-md border border-forge-800/50">
                      {evt.day}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="font-medium text-forge-100 mb-1.5 text-base">{evt.name}</div>
                  <div className="text-xs text-forge-500 flex items-center gap-1.5 mb-3 font-mono">
                    <Clock className="w-3 h-3" />
                    {evt.time}
                  </div>
                  <p className="text-xs text-forge-500 leading-relaxed mb-5 flex-1">{evt.detail}</p>
                  <button
                    ...
                    className={`w-full text-xs font-medium px-3 py-2.5 rounded-lg transition-all duration-300 ${
                      rsvpStatus[evt.day] === 'success'
                        ...
                        : 'bg-forge-800/60 hover:bg-forge-800 text-forge-200 border border-forge-700/50 hover:border-forge-600'
                    }`}
                  >
        ...
        <div className="mt-12 flex flex-wrap gap-3 justify-center md:justify-start">
          <div className="flex items-center gap-2 text-xs text-forge-400 bg-forge-900/30 border border-forge-800/40 rounded-full px-4 py-2">
            <Trophy className="w-3.5 h-3.5 text-ember-500/70" />
            <span>Monthly tournaments with prize support</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-forge-400 bg-forge-900/30 border border-forge-800/40 rounded-full px-4 py-2">
            <Palette className="w-3.5 h-3.5 text-ember-500/70" />
            <span>Free paint-n-take demos on Sundays</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-forge-400 bg-forge-900/30 border border-forge-800/40 rounded-full px-4 py-2">
            <Dice5 className="w-3.5 h-3.5 text-ember-500/70" />
